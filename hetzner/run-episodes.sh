...
# ─── начало блока «чистка замечаний» (проверка: scripts/episodes-notify.test.mjs)
# ЗАМЕЧАНИЯ ПИШУТСЯ, ТОЛЬКО КОГДА ЕСТЬ О ЧЁМ СКАЗАТЬ, А ЛЕЖАТ ВЕЧНО.
# `notify.md` пишет расшифровка, `notify-covers.md` — вырезание персонажей;
# оба закрыты .gitignore, то есть переживают заход и следующий, и десятый.
# 28.08.2026 из-за этого пришло письмо «Расшифрован ep-153» о выпуске,
# расшифрованном НАКАНУНЕ: расшифровок в тот день не прибавилось вовсе,
# в письмо уехал вчерашний файл. Денег это не стоило — платный шаг
# отработал правильно и ep-153 пропустил, — но письмо соврало,
# а письму робота верят.
#
# «Сказать нечего» обязано выглядеть пустым местом, а не прошлым разговором.
rm -f notify.md notify-covers.md
# ─── конец блока «чистка замечаний»
...
# ─── начало блока «что рассказать» (проверка: scripts/episodes-notify.test.mjs)
# Знаки & < > экранируются: письмо уходит с parse_mode=HTML, и на кривой
# разметке телеграм отвечает отказом — письмо не приходит ВОВСЕ. А в замечаниях
# лежат заголовки выпусков, то есть чужой текст, где эти знаки законны.
escape_html() { sed -e 's/&/\&amp;/g' -e 's/</\&lt;/g' -e 's/>/\&gt;/g'; }

DETAILS=""
[ -s notify.md ] && DETAILS="$(head -c 700 notify.md | escape_html)"
[ -z "$DETAILS" ] && [ -s notify-covers.md ] && DETAILS="$(head -c 700 notify-covers.md | escape_html)"
# ─── конец блока «что рассказать»
