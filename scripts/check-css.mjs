 ...
 * Имена файлов стилей в проекте уже меняли (задача 22, снятие хеша) —
 * поменяются ещё раз или переедут, и без этих чисел проверка начнёт отвечать
 * «всё хорошо» на любой вопрос, а отличить это будет нечем.
 */
const охват = { файловСтилей: 0, страниц: 0, стилейВСтранице: 0, правил: 0, объявлений: 0, токеновЦвета: 0 };
...
охват.токеновЦвета = palette.size;
...
охват.объявлений = decls.length;

// ── ЧТО ИМЕННО РАЗОБРАНО ───────────────────────────────────────────────────
// Число, которое вдруг стало нулём, видно сразу. Без этой строчки «пусто»
// у сломанной проверки и у здоровой выглядит одинаково.
console.log(
	`Разобрано: файлов стилей ${охват.файловСтилей}, страниц ${охват.страниц}, ` +
		`стилей внутри страниц ${охват.стилейВСтранице}, правил ${охват.правил}, ` +
		`объявлений ${охват.объявлений}, цветов в палитре ${охват.токеновЦвета}.`,
);

// ПОЛ ОХВАТА. Ноль — это не «чисто», это «не смотрели». Порог именно ноль:
// он не требует подбора и не устаревает от роста сайта.
let ослепла = false;
for (const [что, сколько] of [
	['файлов стилей', охват.файловСтилей],
	['страниц', охват.страниц],
	['правил', охват.правил],
	['объявлений', охват.объявлений],
	['цветов в палитре', охват.токеновЦвета],
]) {
	if (сколько === 0) {
		console.log(`ПРОВЕРКА ОСЛЕПЛА: ${что} разобрано 0 — это не «пусто», это «не смотрели».`);
		ослепла = true;
	}
}
console.log();
...
if (bad > 0 || ослепла) process.exitCode = 1;
