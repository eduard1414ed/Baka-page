...
// НЕВИДИМЫЕ ЗНАКИ, ПРИЕХАВШИЕ С ЧУЖИХ САЙТОВ. Мягкий перенос (U+00AD), нулевой
// пробел, «неразрывный ноль» и метка порядка байтов попадают в текст, когда
// заголовок копируют с веб-страницы: сайты расставляют мягкий перенос, чтобы
// браузер знал, где рвать слово.
//
// В телеграме их не видно, а у нас видно дважды. АДРЕС СТРАНИЦЫ получает лишнее
// тире посреди слова — «бой­кое» превращается в `boy-koe`, «продолже­ние»
// в `prodolzhe-nie`, — и это навсегда: адрес уходит в карту сайта. А поиск
// перестаёт находить слово: «бой­кое» и «бойкое» для движка разные слова.
//
// Замер архива: 12 заголовков, все скопированы с сайтов («Кайдзю № 8»,
// «Гачиакута», «Призрак в доспехах»). В ТЕЛАХ постов таких знаков нет
// ни одного — тело автор набирает руками.
//
// Соединителя U+200D здесь нет НАРОЧНО: он держит вместе составные эмодзи,
// и снятие его разорвало бы 👩‍🌾 на две картинки. Из заголовка его и так
// убирает EMOJI выше.
const INVISIBLE = /[­​⁠﻿]/gu;
...
export function extractTitle(entities) {
	...
	const title = squeeze(stripEmoji(raw).replace(INVISIBLE, ''));
