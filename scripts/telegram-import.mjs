...
export function guessCategory(entities) {
	const urls = urlsOf(entities);
	if (YOUTUBE_HOSTS.some((host) => urls.some((url) => url.includes(host)))) return 'videoessay';
	return 'note';
}
...
export function renderPost(built, { cover = '', gallery = [] } = {}) {
	const blocks = gallery.map((src) => `::image{src="${src}" alt=""}`);
	const text = [blocks.join('\n'), built.body].filter(Boolean).join('\n\n');
