...
/**
 * Адрес нашего материала, ОБЪЯВЛЕННОГО ГОЛОЙ ССЫЛКОЙ В КОНЦЕ ПОСТА.
 * `null` — такой ссылки нет, значит и анонса нет.
 *
 * ЗАЧЕМ ТАК УЗКО. Ссылку на свой материал заказчик ставит ДВУМЯ разными
 * способами, и способ говорит о назначении:
 *
 *   словом по ходу рассказа  — «как я писал в прошлом посте» — это сноска;
 *   голой строкой в конце    — «вот он, идите читать» — это анонс.
 *
 * Замер живых данных (посты №4154–4180, 35 ссылок на наш сайт): голых пять,
 * и все пять стоят ПОСЛЕДНИМ куском поста — после них ноль знаков. Остальные
 * тридцать — словом внутри тела. Ни одного исключения ни в ту, ни в другую
 * сторону. Порога тут поэтому нет вовсе: «в конце» значит «дальше пусто».
 *
 * И РАЗЛИЧАТЬ ИХ НИЧЕГО НЕ СТОИТ: голую ссылку телеграм отдаёт куском
 * `link`, у которого текст и есть адрес, а ссылку словом — куском
 * `text_link` с отдельным адресом. Так делает и бот, и выгрузка, и разбор
 * страницы канала (`bare(shown) === bare(entity.url)` в telegramWebPost.mjs).
 * Три источника, одна форма — своего распознавания писать не пришлось.
 *
 * ОДНОЙ «ГОЛОЙ В КОНЦЕ» МАЛО, и это тоже из данных: у №4156 последним куском
 * стоит голая ссылка на ЧУЖОЙ сайт (рецензия на кинопоиске). Поэтому адрес
 * всё равно обязан быть нашим и обязан вести на страницу материала.
 */
export function announcedMaterialSlug(entities) {
	const куски = entities ?? [];
	for (let i = куски.length - 1; i >= 0; i -= 1) {
		const кусок = куски[i];
		// Хвостовые переводы строк пропускаем: в живых данных их нет, но пост,
		// у которого после ссылки остался перенос, — это тот же самый пост.
		if (!(кусок.text ?? '').trim()) continue;
		// Первый непустой кусок с конца обязан быть ГОЛОЙ ссылкой. Ссылка
		// словом (`text_link`) последним куском тоже бывает — так кончаются
		// посты со списком площадок, — и анонсом она не делает.
		if (кусок.type !== 'link') return null;
		return ourMaterialSlugs([кусок])[0] ?? null;
	}
	return null;
}
...
export function skipReason(post, { нашиМатериалы } = {}) {
	...
	// НАШ САЙТ СПРАШИВАЕТСЯ ТРЕМЯ ВОПРОСАМИ, А НЕ ОДНИМ: стоит ли ссылка
	// ГОЛОЙ СТРОКОЙ В КОНЦЕ поста, ведёт ли она на страницу МАТЕРИАЛА
	// и есть ли этот материал у нас. Ссылка словом по ходу рассказа, ссылка
	// на карточку тайтла, ссылка на главную — постом быть не мешают.
	// Ошибка здесь несимметрична, и потому все три вопроса задаются строго:
	// лишний черновик заказчик увидит и удалит, а молча выброшенный пост
	// не появится нигде и узнать о нём неоткуда.
	const объявлен = announcedMaterialSlug(entities);
	if (объявлен && нашиМатериалы.has(объявлен)) return `анонс уже существующего материала (/posts/${объявлен})`;
