...
//
// ПАДЕЖНЫЕ ФОРМЫ СЧИТАЮТСЯ ЗАОДНО, отдельного шага у них нет: их делает
// writeAnimeEntry в scripts/anime-lib.mjs — то есть ровно там же, где решается
// русское название тайтла (тз/11, часть A). Роботу, которому нечего добирать,
// это не стоит ничего: словари морфологии грузятся только при записи.
//
