...
//
// ФАЙЛА .env МОЖЕТ НЕ БЫТЬ ВОВСЕ, И ЭТО ЗАКОННОЕ СОСТОЯНИЕ, А НЕ ПОЛОМКА.
// На машине заказчика ключи лежат файлом; на Hetzner их подаёт systemd
// переменными окружения из /root/baka-secrets/analytics.env — ровно так же,
// как всем остальным роботам (задача 21). Класть .env в рабочую копию
// на сервере не стали нарочно: файл в папке, где работает git, однажды
// попадёт в чужой коммит, а переменная окружения — никогда.
//
// ФАЙЛ СИЛЬНЕЕ ОКРУЖЕНИЯ. Забытая экспортированная переменная не имеет права
// молча перебить то, что записано руками: окружение только ДОБИРАЕТ то,
// чего в файле нет.
export function ключи() {
	const карта = {};
	const путь = join(КОРЕНЬ, '.env');
	if (existsSync(путь)) {
	...
	for (const имя of ['YM_OAUTH_TOKEN', 'YM_COUNTER_ID', 'GA_PROPERTY_ID', 'GA_KEY_FILE', 'ANALYTICS_DIR']) {
		if (!карта[имя] && process.env[имя]) карта[имя] = process.env[имя];
	}
	return карта;
}
