...
// ЭТИ ДВА ЧИСЛА — ЗАМЕР, А НЕ ЗАСЛОН, И ЭТО ПРАВКА 12 АВГУСТА 2026.
//
// Раньше тут стояло `gainPosts.mentions === 57` — воспроизведение числа,
// названного заказчику 11 августа. Утверждение было верным ровно один день.
// В тот же вечер заказчик завёл из экрана кандидатов новые тайтлы, справочник
// вырос с 53 до 424, и часть мест, достававшихся «Фрирен» единолично, забрали
// соседи: стало 45. Ни одно из них не поломка — это работа заказчика.
//
// А стоит эта проверка ЗАСЛОНОМ перед применением решений. То есть успешная
// работа человека заперла ему же механизм, которым он её делает: кнопка
// «Применить решения» отвечала письмом «✗✗ ПРОВЕРКИ НЕ ПРОШЛИ». Ровно те же
// грабли, что 11 августа с «Баки!» и со «154 столкновениями» (СТАТУС.md,
// хвост 57), и снова пропущенные — правило вывели, а по всем местам не прошли.
//
// ПРАВИЛО: утверждение, растущее вместе с данными, — это ЗАМЕР. Печатает числа,
// ничего не роняет. А замер от поломки бракуют законы ниже, и они берут его
// в клещи С ДВУХ СТОРОН: «вписанное второй раз даёт ноль» ловит замер, который
// всегда отвечает большим числом, а «в одиночку больше, чем рядом со
// справочником» — тот, который всегда отвечает нулём. Абсолютное число для
// этого не нужно вовсе.
console.log(`  замер    «Фрирен» в постах: ${gainPosts.mentions} упоминаний в ${gainPosts.texts} постах`);
console.log(`  замер    «Фрирен» в расшифровках: ${gainTr.mentions} упоминаний в ${gainTr.texts} выпусках`);
console.log(`  замер    11 августа при 53 тайтлах было названо 57 и 192; сейчас тайтлов ${entries.length}.`);
console.log('  замер    расхождение — не поломка: чем больше справочник, тем больше мест забирают соседи.');

// А ВОТ ЭТО — ЗАКОН, И ОТ РАЗМЕРА СПРАВОЧНИКА ОН НЕ ЗАВИСИТ. Название, которого
// в вариантах нет, а в архиве оно есть сотнями, обязано дать прибавку больше
// нуля. Ноль тут означал бы сломанный замер, а не выросший справочник.
check(
	'известное название даёт прибавку больше нуля и в постах, и в расшифровках',
	gainPosts.mentions > 0 && gainTr.mentions > 0,
	`посты ${gainPosts.mentions}, расшифровки ${gainTr.mentions}`,
);
