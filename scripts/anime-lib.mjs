...
export async function writeAnimeEntry(slug, source, result) {
	...
	const entry = {
		...
		// Франшиза приходит из источника; если источник её не отдал (запасной
		// AniList франшиз не знает вовсе), берём прежнюю — терять уже добытое
		// родство не за что. `null` тут законное значение и означает одиночку,
		// поэтому проверяем `!== undefined`, а не на истинность.
		...((result.franchise ?? previous?.franchise) !== undefined && {
			franchise: result.franchise ?? previous?.franchise,
		}),
		// Тип произведения — по тому же правилу, что и франшиза строкой выше:
		// источник отдал — берём его, не отдал (запасной AniList типов
		// в нашем виде не знает) — оставляем прежний. `null` законное значение
		// и означает «тип не назван», поэтому проверяем `!== undefined`.
		// Не перенеси его здесь — и ближайшее обновление данных молча вернуло
		// бы все 658 страниц тайтлов в безликий `CreativeWork`.
		...((result.kind ?? previous?.kind) !== undefined && {
			kind: result.kind ?? previous?.kind,
		}),
		...
		// ВТОРАЯ ГАЛОЧКА — «не искать в расшифровках» (`noSpeech`, 14 августа
		// 2026) — переносится по тому же доводу и была здесь забыта: правило
		// написали, а по всем местам не прошли (ревизия 15 августа 2026).
		// Цена пропуска та же, что у соседки, только заметнее: у «Монстра»
		// и «Города» это 89 и 220 ложных упоминаний в живой речи, и вернулись
		// бы они молча — при первом же обновлении данных из источника.
		...(previous?.noSpeech === true && { noSpeech: true }),
		// ТРЕТЬЕ РЕШЕНИЕ ЧЕЛОВЕКА — «показывать ли страницу поисковикам»
		// (`forceIndex`, 30 августа 2026). В источниках его не существует,
		// переносится по тому же доводу, что и две галочки выше. Не перенеси
		// его здесь — и обновление данных молча вернуло бы в поиск страницу,
		// которую заказчик оттуда убрал, либо убрало ту, что он оставил.
		// Значений три, и `false` — законное: проверяем на `undefined`.
		...(previous?.forceIndex !== undefined && { forceIndex: previous.forceIndex }),
