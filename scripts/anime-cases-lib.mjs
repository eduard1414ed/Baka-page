...
/**
 * ЧТО СТОИТ ПОСЛЕ ГОЛОВЫ И СКЛОНЯЕТСЯ ВМЕСТЕ С НЕЙ.
 *
 * Две вещи, и обе живут справа от главного слова:
 *
 * 1. ПРИЧАСТНЫЙ ОБОРОТ И ОПРЕДЕЛЕНИЕ ПОСЛЕ ЗАПЯТОЙ. «Клинок, рассекающий
 *    демонов» — «рассекающий» согласовано с «Клинком» и обязано склоняться
 *    вместе с ним. Прежде склонялась только голова, и выходило «Клинка,
 *    рассекающий демонов»: строка, в которой одна половина в падеже, а вторая
 *    в именительном. У этого тайтла пять карточек-сезонов, то есть пять таких
 *    строк, и все они друг с другом сталкивались.
 *
 * 2. ПРИЛОЖЕНИЕ ЧЕРЕЗ ДЕФИС. «Папаши-дружбаны», «Дракон-горничная» — вторая
 *    часть склоняется ТОЖЕ, но по СВОЕМУ роду, а не по роду головы: «Дракона-
 *    горничной», а не «Дракона-горничного». Поэтому у неё стоит пометка `own`.
 *    Дефис требуется вплотную, без пробелов: пробел вокруг — это уже тире,
 *    то есть другая часть названия.
 *
 * ОБА СЛУЧАЯ БЕРУТ ТОЛЬКО СОГЛАСОВАННОЕ. Несогласованное слово справа — это
 * зависимое в своём падеже («демонов», «госпожи Кобаяси»), и трогать его
 * нельзя: ровно от этого «Атака титанов» когда-то становилась «атакой титана».
 *
 * @returns {{ index: number, parse: object, own: boolean }[]}
 */
function findTail(head, headParse, words, parsed, chunks, wordAt) {
	const tail = [];
	const headIsNoun = String(headParse.tag.POST) === 'NOUN';

	for (let i = head + 1; i < words.length; i++) {
		const parses = parsed[i];
		if (!parses.length) break;

		// Приложение через дефис: дефис вплотную и существительное за ним.
		const между = chunks.slice(wordAt[i - 1] + 1, wordAt[i]).join('');
		const черезДефис = между === '-' && headIsNoun;
		const noun = nominativeAs(parses, NOUN_ONLY);
		if (черезДефис && noun && noun.tag.NMbr === headParse.tag.NMbr) {
			tail.push({ index: i, parse: noun, own: true });
			continue;
		}

		// Согласованное определение справа от головы. ТОЛЬКО СЛОВАРНОЕ:
		// незнакомое слово словарь охотно принимает за прилагательное, и справа
		// от головы почти всегда стоит ИМЯ — «Признание Фумико», «Бродяга
		// Кэнсин», «Юная революционерка Утэна». Догадка превращала их
		// в «Фумика», «Кэнсиной», «Утэной». Замер по справочнику: все нужные
		// обороты словарные («рассекающий», «покорившая», «закрывающая»),
		// все ложные — угаданные по концу слова.
		//
		// У приложения через дефис (выше) догадка разрешена: там ограничение
		// держит сам дефис, и «Биско-ржавоед» с «Папашами-дружбанами» иначе
		// не склонялись бы вовсе.
		const adj = nominativeAs(parses, DEFINING);
		if (adj && String(adj.parser) === 'Dictionary' && agrees(adj, headParse)) {
			tail.push({ index: i, parse: adj, own: false });
			continue;
		}
...
/**
 * РАЗБОР, ПО КОТОРОМУ СКЛОНЯТЬ НЕЛЬЗЯ, — И ЭТО ВИДНО ДО СКЛОНЕНИЯ.
 *
 * Незнакомое слово словарь разбирает, отрезав от него начало: у «Торадора»
 * он откусывает «тора» и склоняет «дору», у «Псевдогарема» — «псевдо»,
 * у «Суперпринцессы» — «супер». Отрезанное при склонении НЕ ВОЗВРАЩАЕТСЯ:
 * выходит «Доры!», «Гарема», «Принцессы Кагуя!». Строка на вид правильная,
 * а ищет она по архиву чужое — «Гарем» звучит в разговорах про жанр.
 *
 * Второй такой разбор — через «заикание» и опечатку: «Гуррен» словарь читает
 * как «Гурен» с лишней буквой и склоняет «Гурена».
 *
 * Признак берём У САМОГО РАЗБОРА (`parser`, `stutterCnt`, `typosCnt`),
 * а не сравнением букв в результате. Сравнение букв тут врёт: у «любовь» →
 * «любви» и «парень» → «парня» беглая гласная меняет слово ВНУТРИ, и любая
 * проверка «первые буквы совпали» объявляет порчей законную форму. Отчёт
 * (`brokenWords`) сравнивает буквы намеренно — он для глаз и ругается щедро;
 * приговор так выносить нельзя.
 *
 * ПРЕДСКАЗАНИЕ ПО СУФФИКСУ (`SuffixKnown`) СЮДА НЕ ВХОДИТ. Оно тоже догадка,
 * но чаще верная: «Эль Касадора», «Вайолет Эвергардена» — правильные формы,
 * и выбросить их значило бы выбросить пользу вместе с мусором.
 */
function unreliableParse(parse) {
	if (!parse) return null;
	if (String(parse.parser ?? '').startsWith('Prefix')) return 'словарь отрезал начало слова';
	if (parse.stutterCnt > 0 || parse.typosCnt > 0) return 'разбор через опечатку или удвоенную букву';
	return null;
}

/** Есть ли у слова и его новой формы хоть одна общая буква в начале. */
function sameStart(words, wordAt, chunks, out) {
	for (let i = 0; i < words.length; i++) {
		const было = words[i];
		const стало = out[wordAt[i]];
		if (было === стало) continue;
		if (!было || !стало) continue;
		if (fold(было)[0] !== fold(стало)[0]) return false;
	}
	return true;
}
...
export function inflectTitle(titleRu) {
	...
	// НАЗВАНИЕ, НАЧИНАЮЩЕЕСЯ НЕ С ИМЕННОЙ ГРУППЫ, НЕ СКЛОНЯЕТСЯ ВОВСЕ.
	// «Танцуй, танцуй, танцор», «Пощади меня, великий господин!», «Ты умеешь
	// хранить секреты?» — это не именная группа, а фраза, и склонять её целиком
	// нельзя: морфология находит голову где-то в середине и склоняет ЕЁ, оставляя
	// начало как было. Выходит «Танцуй, танцуй, танцора» — строка, которой
	// не скажет никто и не напишет тоже.
	//
	// Правило было и раньше, но жило только в отчёте (`suspiciousForms`): формы
	// помечались подозрительными и всё равно ЗАПИСЫВАЛИСЬ. Пометка «посмотрите
	// глазами» у робота значит «не применять», а тут применялось. Теперь спрашивает
	...
	// НЕНАДЁЖНЫЙ РАЗБОР — У ГОЛОВЫ ИЛИ У ЛЮБОГО ОПРЕДЕЛЕНИЯ — ОТМЕНЯЕТ ВСЁ
	// НАЗВАНИЕ, а не одно слово: половина названия в падеже при второй половине
	// в именительном хуже, чем ничего.
	for (const parse of [headParse, ...modParses, ...tail.map((t) => t.parse)]) {
	...
	// Проверяем и определения: «Обещанный Неверленд» — голова тут «Неверленд»,
	// а страдательное причастие стоит перед ней, и словарь превращал его
	// в действительное: «Обещавшего Неверленда».
	for (const parse of [headParse, ...modParses, ...tail.map((t) => t.parse)]) {
		if (/\bpssv\b/.test(String(parse.tag))) return [];
	}
	...
	for (let i = head + 1 + tail.length; i < words.length; i++) {
	...
	for (const grammeme of CASES) {
		...
		// Хвост: определение справа склоняется по роду головы, приложение
		// через дефис — по своему собственному.
		for (const t of tail) {
			const свои = t.own
				? modGrammemes(grammeme, t.parse.tag.GNdr, t.parse.tag.NMbr, t.parse.tag.ANim)
				: forMods;
			const наборы = t.own && String(t.parse.tag.POST) === 'NOUN'
				? [[grammeme, t.parse.tag.NMbr].filter(Boolean), [grammeme]]
				: свои;
			const склонённое = tryInflect(t.parse, наборы);
			if (!склонённое) {
				ok = false;
				break;
			}
			out[wordAt[t.index]] = keepCase(words[t.index], склонённое.word);
		}
		if (!ok) continue;

		// СУППЛЕТИВ: СЛОВО И ЕГО «ФОРМА» НЕ ИМЕЮТ НИ ОДНОЙ ОБЩЕЙ БУКВЫ В НАЧАЛЕ.
		// «Первый» словарь возводит к «один» и честно склоняет его: «Первый шаг»
		// становился «Одного шага». Порог — ровно одна буква, и он замерен,
		// а не назначен: у законных чередований общее начало есть всегда, даже
		// у самых коротких («ложь» → «лжи» — одна буква, «любовь» → «любви» — три).
		if (!sameStart(words, wordAt, chunks, out)) continue;
