...
// Образец на случай, когда в архиве хештегов не осталось. Это НЕ «пример,
// вписанный именем»: живой пример по-прежнему главный и ищется первым.
// Строка взята из архива в тот день, когда правка ещё не была применена.
const ОБРАЗЕЦ_СТРОКИ = 'Смотрите, что вышло! #аниме #рекомендации';

async function selftest() {
	const posts = await readPostsRaw();

	// НАСТОЯЩУЮ СТРОКУ АРХИВА СПРАШИВАЕМ У САМОГО ПРАВИЛА, А НЕ У РЕГУЛЯРКИ.
	//
	// Прежде тут стояло `HASHTAG.test(строка)`, и это было неверно дважды.
	// Во-первых, `HASHTAG` объявлена с флагом `g` и помнит позицию между
	// вызовами: при переборе строк она то ищет с начала, то с середины,
	// и часть строк объявляет пустыми ни за что. Во-вторых, грубая регулярка
	// не знает про ссылки, код и заголовки — и находила «хештег» в якоре
	// адреса (`…/anime-autumn-2025/#six`), а правило такую строку законно
	// не трогает: пример выходил заведомо непроходным.
	//
	// Спрашиваем `stripHashtags` — тот самый код, который проверяем. Если он
	// говорит «тут есть что убрать», строка годится в пример по определению.
	let realLine = null;
	...
	// ПУСТО — ЭТО НЕ ПОЛОМКА, А СЛЕД СДЕЛАННОЙ РАБОТЫ, и молчать об этом нельзя.
	// Правка применена, хештегов в архиве не осталось, живому примеру взяться
	// неоткуда. Прежде проверка тут ПАДАЛА целиком — то есть переставала быть
	// прогоняемой ровно после того, как сделала своё дело (доревизия задачи 15,
	// находка 21). Красная проверка, которую нечем сделать зелёной, живёт
	// до первой уборки.
	const наЖивом = realLine !== null;
	if (!наЖивом) {
		realLine = ОБРАЗЕЦ_СТРОКИ;
		console.log('  ПРИМЕЧАНИЕ: в архиве не осталось ни одной строки с хештегом — правка');
		console.log('              применена. Проверяю на записанном образце, а не на живом.');
	} else {
		console.log(`  примером взята живая строка архива: ${JSON.stringify(realLine.slice(0, 70))}`);
	}

	const cases = [
		{
			name: наЖивом ? 'настоящая строка архива: хештег убран' : 'записанный образец строки: хештег убран',
			body: `\n${realLine}\n`,
