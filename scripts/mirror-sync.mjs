 ...
 * Отпечаток многочастной загрузки содержит дефис и с обычным MD5 не сравним —
 * такой объект считаем изменившимся и перезаливаем. Своих многочастных
 * загрузок мы не делаем, так что случиться это может только с чужой.
 */
export async function перечислитьБакет() {
	...
	do {
		...
		for (const block of xml.split('<Contents>').slice(1)) {
			...
			if (key) объекты.set(изXML(key), изXML(etag ?? '').replace(/&quot;|"/g, '').toLowerCase());
		}
