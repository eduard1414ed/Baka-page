...
// ── 3. УХОДЫ НА ПЛОЩАДКИ ───────────────────────────────────────────────────
{
	...
	// ── НАЖАТИЕ ПО ЗНАЧКУ ВНУТРИ ССЫЛКИ ────────────────────────────────────
	//
	// ВЫШЕ НАЖИМАЛИ ПО САМОЙ ССЫЛКЕ, А В ЖИЗНИ ТАК НЕ НАЖИМАЮТ. В подвале
	// сайта ссылка на телеграм — это <a> с рисунком внутри:
	//   <a class="social-link" href="https://t.me/podcastbaka" target="_blank">
	//     <svg><path/></svg>
	//   </a>
	// Палец попадает в <path>, и до ссылки код обязан дойти сам, вверх
	// по родителям. Разница не теоретическая: у СЛУЧАЯ ВЫШЕ цель нажатия
	// САМА является ссылкой, и он прошёл бы, даже если бы поиск родителя
	// был сломан вовсе.
	{
		const о = await поднять('ru.bakapodcast.com');
		const ссылка = о.зарегистрировать(new о.Узел(['social-link'], { tag: 'a', href: 'https://t.me/podcastbaka' }));
		const рисунок = ссылка.добавить(new о.Узел([], { tag: 'svg' }));
		const черта = рисунок.добавить(new о.Узел([], { tag: 'path' }));
		о.вызватьНаДокументе('click', { target: черта });
		const пришло = имена(о).find((и) => и === 'go_support' || и === 'go_listen' || и === 'go_social') ?? null;
		проверить('нажатие по ЗНАЧКУ внутри ссылки (как в подвале)', пришло === 'go_social', `ждали go_social, пришло ${пришло ?? 'ничего'}`);
	}
	console.log();
}
