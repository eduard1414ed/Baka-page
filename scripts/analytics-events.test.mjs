...
// ── 2. ТРАНСКРИПТ И ТАЙТЛЫ ─────────────────────────────────────────────────
{
	...
	нажать(о.зарегистрировать(new о.Узел(['tr-time'], { dataset: { seconds: '724' } })));
	проверить('таймкод в расшифровке засчитан', имена(о).includes('transcript_seek'));
	проверить('место — расшифровка', параметры(о, 'transcript_seek').place === 'transcript');
	проверить('секунда записана', параметры(о, 'transcript_seek').seconds === 724);
