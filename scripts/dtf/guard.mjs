...
export function writeGuarded(file, built, { force = false } = {}) {
	// НЕЧЕГО МЕНЯТЬ — НЕ ПИШЕМ ВОВСЕ. Заслон нарочно не считает правкой пустые
	// строки: админка расставляет их по-своему, и иначе он срабатывал бы после
	// каждого сохранения впустую. Но тогда запись всё равно клала на диск СВОЙ
	// вариант пустых строк — и `git diff` показывал правку там, где по существу
	// не изменилось ничего. Собственная пересборка «на всякий случай» тихо
	// перетирала расстановку заказчика: разница невидима глазом и не ловится
	// ни одной проверкой. Поймано тем, что после прогона всех сборщиков
	// `git status` оказался не пуст.
	if (fs.existsSync(file) && handEdited(file, built) === null && fs.readFileSync(file, 'utf8') !== built) return { skipped: true };
