...
async function main() {
	...
	for (const p of plan.slice(0, write ? 0 : 6)) {
		...
		// РОЛИКА У ЗАПИСИ МОЖЕТ НЕ БЫТЬ ВОВСЕ, и это не край, а обычный случай:
		// распознанное эссе получает свою категорию даже без ролика (решение
		// заказчика 13 августа). Прежняя строка спрашивала `p.video.title`
		// безусловно и роняла ВЕСЬ ПОКАЗ TypeError'ом — то есть режим, который
		// запускают первым и ради которого разведка существует. Заметно это
		// становилось только при определённом порядке постов: показываются
		// первые шесть, и до правки среди них такого не попадалось.
		console.log(p.video ? `      → ${p.url}  «${p.video.title}»` : '      → ролика нет — меняется только категория');
