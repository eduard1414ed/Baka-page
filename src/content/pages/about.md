---
lead: Подкаст и медиа об аниме — о новых сериалах, классике, людях индустрии и о том, что скрывается между кадрами.
hostsNote: Кто делает «Баку!»
hosts:
  - name: Эд
    role: Ведущий · сценарист
    photo: /images/uploads/photo_2026-08-05_13-29-57.jpg
    bio: Медиаменеджер, помимо «Баки!» — автор подкаста про комиксы и мангу «Омнибус и танкобон» от Arzamas, текстов об аниме и манге для «Кинопоиска», «Т—Ж» и других изданий.
    urlLabel: Telegram
    url: https://t.me/etsarionov
  - name: Ксюша
    role: Ведущая
    photo: /images/uploads/Без имени-1.jpg
    bio: Digital Project Manager — генерирует, продюсирует и собирает любую идею в готовый медийный проект. А пока никто не видит, перевоплощается в журналиста, пишет статьи и сценарии про аниме.
    urlLabel: Telegram
    url: https://t.me/ksushaotakuet
  - name: Иван
    role: Монтажёр
    photo: /images/uploads/Без имени-2.jpg
    bio: Собирает наши потрясающие видеоверсии и эссе. Помимо этого работает и над десятком других успешных подкастов.
    urlLabel: Telegram
    url: https://t.me/in_da_tresh
listenNote: Бака! в наушниках
socialNote: Бака! в других местах
supportTitle: Поддержать «Баку!»
supportNote: Подписка помогает нам выпускать подкасты, видеоэссе и тексты — и даёт доступ к бонусным выпускам и дополнительным материалам.
contactText: По вопросам сотрудничества, рекламы или участия в выпуске — пишите на почту
contactEmail: litotaprod@gmail.com
---

Мы начали «Баку!» как разговорный подкаст, но со временем проект вырос
в площадку с видеоэссе, большими текстами, заметками и теперь целым сайтом.
Нам интересно не просто советовать тайтлы, а разбирать, как они сделаны,
почему работают и что рассказывают о мире вокруг.

На сайте все форматы связаны между собой: у каждого аниме есть собственная
страница, где собраны наши статьи, видео и выпуски подкаста — вплоть
до конкретных минут разговора, на которых тайтл упоминался.
