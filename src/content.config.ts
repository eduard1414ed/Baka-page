	...
	schema: () =>
		z.object({
			...
			anime: z.preprocess(emptyToUndefined, z.array(reference('anime')).optional()),
				...
				// Категорию при импорте УГАДАЛ РОБОТ, а не выбрал человек.
				// Пока галочка стоит, пост числится спорным; заказчик снимает её,
				// когда согласился с догадкой. Отдельное поле, а не молчание:
				// иначе угаданная категория неотличима от выбранной, и «предлагается,
				// а не проставляется молча» осталось бы словами в ТЗ.
				categoryGuessed: z.preprocess(emptyToUndefined, z.boolean().default(false)),
