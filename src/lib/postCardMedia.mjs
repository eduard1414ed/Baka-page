 ...
 *   4) первая картинка из текста — ровно то, что обещает подсказка поля
 ...
 *
 * СТУПЕНИ «КАДР С ЮТЮБА» БОЛЬШЕ НЕТ. Она читала поле «Ссылка на ролик»
 * у поста и брала оттуда картинку сама; поле снято решением заказчика
 * 15 августа 2026 и было заполнено у нуля постов. У видеоэссе архива
 * кадр ролика лежит настоящим файлом в поле «Обложка» — так его положила
 * задача 19, и это лучше кадра «на лету»: настоящий файл виден ещё
 * и в превью ссылки в телеграме, а взятый по ссылке не виден там вовсе.
 *
...
export async function getCardMedia(post) {
	const { cover, noCover, audioGuid } = post.data;
