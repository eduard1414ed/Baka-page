 ...
 *
 * ПОЛЯ «ЧТО ЭТО» В НАШИХ ДАННЫХ СЕЙЧАС НЕТ — ни у одной из 658 карточек.
 * Shikimori его отдаёт (`kind`: tv, movie, ova, ona, special, music)
 * в том же ответе `GET /api/animes/<id>`, который проект и так запрашивает,
 * но справочник его не сохраняет. Пока поля нет, все страницы получают
 * `CreativeWork` — «произведение», без уточнения. Это честно и валидно,
 * просто менее выразительно, чем `Movie` и `TVSeries`.
 *
 * РАЗБОР НАПИСАН НЕ «НА БУДУЩЕЕ», А ПОТОМУ ЧТО ЗАДАНИЕ ТРЕБУЕТ ОБЕИХ ВЕТОК:
 * «по типу тайтла, если он есть в данных; если типа нет — CreativeWork».
 * Появится поле — страницы станут точнее без единой правки здесь.
 *
 * OVA, ONA, спешлы и клипы намеренно остаются `CreativeWork`: сериал это или
 * фильм, по одному слову `ova` не решается, а гадать в разметке нельзя.
 */
export function animeSchemaType(kind) {
	if (kind === 'movie') return 'Movie';
	if (kind === 'tv' || kind === 'tv_special') return 'TVSeries';
	return 'CreativeWork';
}
