...
// ОТКРЫВАЮЩИЕ — ОТДЕЛЬНЫМ СПИСКОМ, И ЭТО НЕ ОТМЕНА ПРАВИЛА ВЫШЕ. Искать
// упоминания мы по-прежнему беремся, не отличая открывающую от закрывающей:
// в архиве из мессенджера сплошь «Монстр“ с лапкой не с той стороны, и строгость
// там теряла бы настоящие упоминания.
//
// Но у одного вопроса ответ зависит от СТОРОНЫ: «начинается ли тут фраза
// в кавычках». Без этого различения «Боруто», «Фури-кури», «Приоритет чудо-яйца»
// читается так, будто фраза началась на закрывающей кавычке соседа, — и три
// ссылки из 1317 теряли свой номер ни за что (замер 31 августа 2026).
//
// Симметричных (прямая кавычка, апострофы) тут НЕТ намеренно: по ним сторону
// не определить, и молчаливая догадка вернула бы ту же беду. Замер архива:
// все 999 пар вокруг ссылок на тайтлы — ёлочки, так что цена этой строгости нулевая.
const OPENING_QUOTES = new Set(['«', '„', '‹', '「', '『']);

/** Открывает ли этот знак фразу в кавычках. */
export function isOpeningQuote(char) {
	return char !== undefined && OPENING_QUOTES.has(char);
}
...
/**
 * ГДЕ СТОИТ БЛИЖАЙШАЯ КАВЫЧКА — не «есть ли она», а именно где.
 *
 * Отвечать местом, а не «да/нет», понадобилось, когда ссылка на тайтл стала
 * ЗАБИРАТЬ КАВЫЧКИ ВНУТРЬ СЕБЯ (решение заказчика 31 августа 2026,
 * src/plugins/remark-anime.mjs): надстрочный номер обязан стоять ЗА закрывающей
 * кавычкой, а не внутри неё, — «Моб Психо 100»³⁶⁵, а не «Моб Психо 100³⁶⁵».
 * Чтобы кавычки попали под подчёркивание, надо знать, где именно они стоят.
 *
 * Спрашивать при этом обязательно ЭТО правило, а не своё: второе разошлось бы
 * с правилом галочки «только в кавычках», и ссылка забирала бы кавычки там,
 * где упоминание кавычками не считается, — или наоборот.
 *
 * @returns {number} место кавычки в строке или −1, если её нет.
 */
export function indexOfQuoteNear(text, from, step) {
	for (let i = 0, at = from; i <= QUOTE_TAIL; i++, at += step) {
		const char = text[at];
		if (char === undefined) return -1;
		if (isQuote(char)) return at;
...
function quoteNear(text, from, step) {
	return indexOfQuoteNear(text, from, step) !== -1;
}
