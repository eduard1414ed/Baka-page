 ...
 */
/**
 * ОДИН УКАЗАТЕЛЬ НА ВСЮ СБОРКУ.
 *
 * `buildMentionIndex` разбирает 144 расшифровки и ~750 текстов постов: замер
 * 30 августа 2026 — 11–12 секунд на прогон. До этой обёртки он строился ЗА
 * СБОРКУ ДВАЖДЫ: страницы тайтлов звали его напрямую, а каталог — через свой
 * кэш. Потребителей стало четверо (добавились карта сайта и лог
 * индексируемости), и без общего кэша сборка выросла бы на полминуты.
 *
 * СТОРОЖ ВАЖНЕЕ ЭКОНОМИИ. Ключ кэша — длины коллекций (приём взят
 * у `buildCatalog` и в проекте уже принят). Позови кто-нибудь указатель
 * с ДРУГИМ набором постов — построение случится второй раз, и мы об этом
 * скажем вслух. Это не про скорость: разметка страницы и карта сайта обязаны
 * отвечать на вопрос «индексируется ли страница» ОДИНАКОВО, а разные наборы
 * постов у них — это ровно тот противоречивый сигнал поисковику, который
 * задание называет худшим из возможных исходов.
 *
 * @returns {ReturnType<typeof buildMentionIndex>}
 */
let общийУказатель = null;
let построенийЗаПроцесс = 0;

export function sharedMentionIndex({ posts, transcripts, animeList }) {
	const ключ = `${posts.length}:${transcripts.length}:${animeList.length}`;
	if (общийУказатель?.ключ === ключ) return общийУказатель.index;

	построенийЗаПроцесс += 1;
	if (построенийЗаПроцесс > 1) {
		console.warn(
			`⚠ Указатель упоминаний построен ${построенийЗаПроцесс}-й раз за сборку ` +
				`(ключ «${ключ}», прежний «${общийУказатель?.ключ ?? 'нет'}»). ` +
				'Значит кто-то зовёт его с другим набором постов — проверьте, ' +
				'что страница тайтла и карта сайта берут одни и те же посты.',
		);
	}

	const index = buildMentionIndex({ posts, transcripts, animeList });
	общийУказатель = { ключ, index };
	return index;
}
