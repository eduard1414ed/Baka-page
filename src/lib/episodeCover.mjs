...
/**
 * Обложка выпуска для превью в соцсетях — jpeg на НАШЕМ домене.
 *
 * Зачем отдельный файл, если обложка уже есть на хостинге подкаста и она jpeg:
 * телеграм её так и не показал. Прямых доказательств, почему именно, из нашего
 * кода не добыть, но подозрительных отличий от работающих картинок было три
 * сразу — чужой домен, `Cache-Control: no-cache` (а телеграм картинку превью
 * скачивает к себе) и вес 1,5 МБ при 2000×2000. Своя копия снимает все три
 * разом: наш домен, обычные заголовки, 1200 px и около 100 КБ.
 *
 * Сам файл делает сборка из УЖЕ СКАЧАННОЙ webp-копии — заново с хостинга
 * ничего не тянется (src/plugins/optimize-uploads-integration.mjs). Делается
 * только для выпусков, которые опубликованы: по файлу на выпуск, а их будет
 * под полторы сотни.
 *
