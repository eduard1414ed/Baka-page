...
/**
 * Описание материала: своё, если заказчик его написал, иначе — из текста.
 *
 * ЗОВУТ ОТСЮДА ВСЕ ЧЕТВЕРО: подводка на карточке, описание страницы для
 * поисковика, превью ссылки в мессенджере и лента RSS. Правило «своё побеждает»
 * должно жить в одном месте — иначе в ленте окажется одно описание, а в поиске
 * другое, и заметить это можно будет только случайно.
 *
 ...
 * строками средствами CSS, а поисковик обрежет своё сам.
 *
 * @param {{ body?: string, data?: { description?: string } }} post
 * @param {string} fallback Что вернуть, если нет ни своего описания, ни текста.
 * @param {{ maxLength?: number }} [options] Предел для описания ИЗ ТЕКСТА.
 * @returns {string}
 */
export function postDescription(post, fallback, options) {
	const own = String(post?.data?.description ?? '').trim();
	if (own) return own;

	return excerptFromBody(post?.body, fallback, options);
}
