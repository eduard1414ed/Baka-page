...
function linkListData(items) {
	return {
		...
		hProperties: {
			...
			style: `--inset-chars: ${longestLabelChars(items)}`,
		},
...
/**
 * ДЛИНА САМОЙ ДЛИННОЙ ПОДПИСИ ВРЕЗКИ, В ЗНАКАХ. По ней столбец названий
 * раздвигается под содержимое — до потолка, который держит `global.css`.
 *
 * СЧИТАЕМ ЗНАКИ, А НЕ ПИКСЕЛИ, И ЭТО НЕ ПРИДИРКА. Кегль технического слоя
 * и трекинг живут в стилях; посчитай мы тут пиксели — те же числа появились
 * бы вторым описанием и разъехались бы с первой правкой кегля. Перевод знаков
 * в ширину делает CSS, у которого эти числа и так есть.
 *
 * ШИРИНА ЗНАКА ОДИНАКОВА У ВСЕХ: гарнитура моноширинная, замер обоих её
 * начертаний (fontTools) даёт ровно 0.6 em и на латинице, и на кириллице,
 * и на знаках препинания. Подпись набрана капслоком, но в моноширинной
 * гарнитуре это ширины не меняет.
 *
 * ПОЧЕМУ НЕ ОБЩАЯ СЕТКА НА ВСЮ ВРЕЗКУ (тогда столбец подобрался бы сам,
 * без счёта): строки второй врезки — оглавления выпуска — это `<button>`
 * внутри `<li>`, до сетки списка они не дотягиваются, а `display: contents`
 * на кнопке отняло бы у неё коробку. Перестройка разметки обоих блоков стоит
 * больше, чем эта строчка.
 */
function longestLabelChars(items) {
	let longest = 0;
	for (const item of items) longest = Math.max(longest, (item.label ?? '').length);
	return longest;
}
